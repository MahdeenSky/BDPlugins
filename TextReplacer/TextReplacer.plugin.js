...
function getSettings() {
  // if there are no settings saved, save the default settings
...
module.exports = (meta) => ({
    ...
    BD.Patcher.before(MessageActions, "sendMessage", (_, args) => {
      ...
      for (let i = 0; i < getSettings().regexes.length; i++) {
        ...
        msg.content = msg.content.replace(
          new RegExp(definedRegex.regex, "g"),
  ...
  getSettingsPanel() {
    return BdApi.React.createElement(
      Settings.Provider,
      { value: this },
      BdApi.React.createElement(this.SettingsPanel),
    );
  },
  ...
  SettingsPanel() {
    ...
    // if getSettings() is undefined, set it to the default settings
    if (!getSettings()) {
